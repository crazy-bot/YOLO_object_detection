...
parser.add_argument("--model_config_path", type=str, default="config/mydata_yolov4.cfg", help="path to model config file")
parser.add_argument("--data_config_path", type=str, default="config/mydata.data", help="path to data config file")
parser.add_argument("--weights_path", type=str, default="/data/Guha/construction/800.weights", help="path to weights file/ can use darknet53.conv.74")
...
parser.add_argument("--checkpoint_dir", type=str, default="/data/Guha/AEC/checkpoints", help="directory where model checkpoints are saved")
...
test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=opt.batch_size, shuffle=False, num_workers=opt.n_cpu)
...
for epoch in range(0,opt.epochs):
    ...
    for batch_i, (imgpath, imgs, targets) in enumerate(dataloader):
        #print(imgpath)
    ...
    if(mAP > best_mAP):
        best_mAP = mAP
        model.save_weights("%s/mydata_best.weights" % (opt.checkpoint_dir))
